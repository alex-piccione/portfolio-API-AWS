...
type CurrencyRepository (connectionString:string, database:string, collectionName:string) =
    inherit CrudRepository<Currency>(connectionString, database, collectionName, (fun x -> x.Code))

    new (connectionString:string) = CurrencyRepository(connectionString, RepositoryBase<_>.DefaultDatabase, "Currency")
